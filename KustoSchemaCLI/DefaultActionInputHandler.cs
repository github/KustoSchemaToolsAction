    ...
    public static class DefaultActionInputHandler
    {
        public static async ValueTask StartAnalysisAsync(ActionInputs options, KustoSchemaHandler handler, ILogger logger)
        {
            ...
            try
            {
                ...
                {
                    case "diff":
                        var response = await handler.GenerateDiffMarkdown(Path.Combine(options.Path, options.Cluster), options.Database);
                        ...
                        break;
                    case "import":
                        await handler.Import(Path.Combine(options.Path, options.Cluster), options.Database,  options.IncludeColumns.ToLower() == "true");
                        break;
                    case "apply":
                        await handler.Apply(Path.Combine(options.Path, options.Cluster), options.Database);
                        break;
                    case "debug":
